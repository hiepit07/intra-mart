...
<%--
  ...
  日付                 指示者      担当            内容
  -------------------------------------------------------------------------
  2015/10/27 1.00                  ACT)ANZAI       新規作成
  -------------------------------------------------------------------------
...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/sei/sei0102d01.css" />
...
<script src="<c:url value="/resources/js/sei/sei0102d01.js" />" type="text/javascript"></script>

<tiles:insertDefinition name="menuTemplate">
	<tiles:putAttribute name="body">
		<form:form action="proc" method="POST" cssErrorClass="error-input" modelAttribute="FormSei0102d01" id="FormSei0102d01">

			<div id="wrapper">
			
				<!-- エラー表示部 -->
				<c:choose>
					<c:when test="${FormSei0102d01.msgErrorLevel}">
						<%-- エラー --%>
						<div id="messError" class="messError_error">
					</c:when>
					<c:when test="${FormSei0102d01.msgWarningLevel}">
						<%-- 警告 --%>
						<div id="messError" class="messError_warning">
					</c:when>
					<c:when test="${FormSei0102d01.msgInfoLevel}">
						<%-- 情報 --%>
				...
				<div class="content">
						...
						<div class="data_conditions_area_cont">
							...
							<div class="condition_cont">
								<table id="tblDataCondition" class="tbl_data_condition">
									<c:if test="${FormSei0102d01.sysAdminFlag}">
										<tr>
											<td class="align_right"><span id="ddlShozokuLabel">所属事業所</span></td>
											<td class="td_width3 orange_line"></td>
											<td>
												<c:choose>
													<c:when test="${FormSei0102d01.notFoundFlag}">
									...
									<tr>
										...
										<td class="align_right td_width150">事務担当者</td>
										<td class="td_width3"></td>
										<td class="td_width250">
											<c:choose>
												<c:when test="${FormSei0102d01.notFoundFlag}">
													<form:input type="text" class="txt_width100 align_left float_left" path="jimuTantoshaCd" id="jimuTantoshaCd" maxlength="8" />
													<img id="btnSearchUser" class="searchIcon float_left" alt="検索" src="${pageContext.request.contextPath}/resources/css/images/search.png">
												</c:when>
												<c:otherwise>
													<form:input type="text" disabled="true" class="txt_width100 align_right float_left" path="jimuTantoshaCd" id="jimuTantoshaCd" maxlength="8" />
													<img id="btnSearchUser" disabled="true" class="searchIcon float_left" alt="検索" src="${pageContext.request.contextPath}/resources/css/images/search_dis.png">
												</c:otherwise>
											</c:choose>
											<div class="width100 float_left mt3"><span id="dispJimuTantoshaName" class="color_blue">${FormSei0102d01.jimuTantoshaName}</span></div>
											<%-- HIDDEN --%>
											<form:hidden id="jimuTantoshaName" path="jimuTantoshaName" />
										</td>
										<td>
											<c:choose>
												<c:when test="${FormSei0102d01.notFoundFlag}">
													<form:checkbox id="shoriTaishoFlag" path="shoriTaishoFlag" /> <label for="shoriTaishoFlag">処理対象のみ表示</label>
												</c:when>
												<c:otherwise>
													<form:checkbox id="shoriTaishoFlag" disabled="true" path="shoriTaishoFlag" /> <label for="shoriTaishoFlag">処理対象のみ表示</label>
												</c:otherwise>
											</c:choose>
										</td>
										<td>
											<c:choose>
												<c:when test="${FormSei0102d01.notFoundFlag}">
													<form:checkbox id="tashaKokyakuFlag" path="tashaKokyakuFlag" /> <label for="tashaKokyakuFlag">他社顧客</label>
												</c:when>
												<c:otherwise>
													<form:checkbox id="tashaKokyakuFlag" disabled="true" path="tashaKokyakuFlag" /> <label for="tashaKokyakuFlag">他社顧客</label>
												</c:otherwise>
											</c:choose>
										</td>
									...
									<tr>
										...
										<td colspan="4">
											...
											<span id="dispSeikyusakiName" class="color_blue">${FormSei0102d01.seikyusakiName}</span>
											<%-- HIDDEN --%>
										...
										<td colspan="3">
											<c:choose>
												<c:when test="${FormSei0102d01.notFoundFlag}">
													<form:checkbox id="seikyuMisoshinFlag" path="seikyuMisoshinFlag" /> <label for="seikyuMisoshinFlag">請求データ未送信のみ表示</label>
												</c:when>
												<c:otherwise>
													<form:checkbox id="seikyuMisoshinFlag" disabled="true" path="seikyuMisoshinFlag" /> <label for="seikyuMisoshinFlag">請求データ未送信のみ表示</label>
												</c:otherwise>
											</c:choose>
										</td>
									</tr>
					...
					<!-- 明細部 -->
					<c:if test="${FormSei0102d01.showListFlag}">
						<div class="data_result_area clear">
							<div class="div_zentai">
								...
								<c:choose>
									<c:when test="${FormSei0102d01.notFoundFlag}">
										<%-- 検索結果0件 --%>
										<div id="divBody" class="divBody height440 align_center tbl_list">
											${notFoundMessage}
										</div>
									</c:when>
									<c:otherwise>
										<div id="divBody" class="divBody height440">
											<table id="tblBody" class="tbl_list">
												...
												<%-- START 請求先情報ループ --%>
												<c:forEach items="${FormSei0102d01.seikyusakiInfoList}" var="seikyusakiInfo" varStatus="status">
													<tr>
														<%-- No --%>
														...
														<%-- 選択 --%>
														<td class="align_center">
															<input type="checkbox" name="seikyusakiInfoList[${status.index}].checkBoxFlag"
														...
														<td class="align_center">
															<c:choose>
																<c:when test="${seikyusakiInfo.zenkaiSeikyuShimeKbn == '2'}">
																	有
																</c:when>
																<c:otherwise>
																	&nbsp;
					...
					<div>
						<c:choose>
							<c:when test="${FormSei0102d01.notFoundFlag}">
								...
								<button id="btnPrintNohinMeisai" disabled="true" class="btn_button_disabled btn_width120 float_left mr100" type="button">納品明細書印刷</button>
							...
							<c:otherwise>
								...
								<button id="btnPrintNohinMeisai" class="btn_button btn_width120 float_left mr100" type="button">納品明細書印刷</button>
			...
			<form:hidden id="condJimuTantoshaCd"     path="condJimuTantoshaCd" />
			<form:hidden id="condShoriTaishoFlag"    path="condShoriTaishoFlag" />
			<form:hidden id="condTashaKokyakuFlag"   path="condTashaKokyakuFlag" />
			<form:hidden id="condSeikyuMisoshinFlag" path="condSeikyuMisoshinFlag" />
