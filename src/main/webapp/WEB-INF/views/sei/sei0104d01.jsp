...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/sei/sei0104d01.css" />
...
<script src="<c:url value="/resources/js/sei/sei0104d01.js" />" type="text/javascript"></script>

<tiles:insertDefinition name="menuTemplate">
	<tiles:putAttribute name="body">
		<form:form action="proc" method="POST" cssErrorClass="error-input" modelAttribute="FormSei0104d01" id="FormSei0104d01">
		
			<div id="wrapper">
				
				<!-- エラー表示部 -->
				<c:choose>
					<c:when test="${FormSei0104d01.msgErrorLevel}">
						<%-- エラー --%>
						<div id="messError" class="messError_error">
					</c:when>
					<c:when test="${FormSei0104d01.msgWarningLevel}">
						<%-- 警告 --%>
						<div id="messError" class="messError_warning">
					</c:when>
					<c:when test="${FormSei0104d01.msgInfoLevel}">
						<%-- 情報 --%>
				...
				<div class="content">
								...
								<table id="tblDataCondition" class="tbl_data_condition">
									...
									<tr>
										...
										<td colspan="4">
											...
											<span id="dispSeikyusakiName" class="color_blue">${FormSei0104d01.seikyusakiName}</span>
											<%-- HIDDEN --%>
					...
					<!-- 明細部 -->
					<c:if test="${FormSei0104d01.showListFlag}">
						<div class="data_result_area clear">
							<p class="text_request">以下の請求先の請求締めを取り消します。よろしければ、実行ボタンを押してください。</p>
							<div class="div_zentai">
								...
								<c:choose>
									<c:when test="${FormSei0104d01.notFoundFlag}">
										<%-- 検索結果0件 --%>
										<div id="divBody" class="divBody height530 align_center tbl_list">
											${notFoundMessage}
										</div>
									</c:when>
									<c:otherwise>
										...
										<div id="divBody" class="divBody height530">
											<table id="tblBody" class="tbl_list">
												...
												<%-- START 請求先情報ループ --%>
												<c:forEach items="${FormSei0104d01.seikyusakiInfoList}" var="seikyusakiInfo" varStatus="status">
