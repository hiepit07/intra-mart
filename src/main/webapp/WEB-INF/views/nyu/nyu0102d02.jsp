...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/nyu/nyu0102d02.css" />
...
<script src="<c:url value="/resources/js/nyu/nyu0102d02.js" />" type="text/javascript"></script>

<tiles:insertDefinition name="defaultTemplate">
	<tiles:putAttribute name="body">
		<form:form action="proc" method="POST" cssErrorClass="error-input" modelAttribute="FormNyu0102d02" id="FormNyu0102d02">
			
			<div id="wrapper">
				
				<!-- エラー表示部 -->
				<c:choose>
					<c:when test="${FormNyu0102d02.msgErrorLevel}">
						<%-- エラー --%>
						<div id="messError" class="messError_error">
					</c:when>
					<c:when test="${FormNyu0102d02.msgWarningLevel}">
						<%-- 警告 --%>
						<div id="messError" class="messError_warning">
					</c:when>
					<c:when test="${FormNyu0102d02.msgInfoLevel}">
						<%-- 情報 --%>
				...
				<div class="content">
							...
							<div class="condition_cont">
								<table class="tbl_data_condition">
									...
									<tr>
										...
										<td class="td_width300">
											<c:choose>
												<c:when test="${FormNyu0102d02.notFoundFlag}">
													<form:input type="text" class="txt_width100 align_right" path="seikyusakiCdFrom" id="seikyusakiCdFrom" maxlength="7" />
												</c:when>
												<c:otherwise>
													<form:input type="text" disabled="true" class="txt_width100 align_right" path="seikyusakiCdFrom" id="seikyusakiCdFrom" maxlength="7" />
												</c:otherwise>
											</c:choose>
											<span>&nbsp;&nbsp;</span>
											～
											...
											<c:choose>
												<c:when test="${FormNyu0102d02.notFoundFlag}">
													<form:input type="text" class="txt_width100 align_right" path="seikyusakiCdTo" id="seikyusakiCdTo" maxlength="7" />
												</c:when>
												<c:otherwise>
													<form:input type="text" disabled="true" class="txt_width100 align_right" path="seikyusakiCdTo" id="seikyusakiCdTo" maxlength="7" />
												</c:otherwise>
											</c:choose>
										</td>
										...
										<td class="td_width300">
											<c:choose>
												<c:when test="${FormNyu0102d02.notFoundFlag}">
													<form:input type="text" class="txt_width100" path="nyukinYoteibiFrom" id="nyukinYoteibiFrom" maxlength="8" />
												</c:when>
												<c:otherwise>
													<form:input type="text" disabled="true" class="txt_width100" path="nyukinYoteibiFrom" id="nyukinYoteibiFrom" maxlength="8" />
												</c:otherwise>
											</c:choose>
											<span>&nbsp;&nbsp;</span>
											～
											...
											<c:choose>
												<c:when test="${FormNyu0102d02.notFoundFlag}">
													<form:input type="text" class="txt_width100" path="nyukinYoteibiTo" id="nyukinYoteibiTo" maxlength="8" />
												</c:when>
												<c:otherwise>
													<form:input type="text" disabled="true" class="txt_width100" path="nyukinYoteibiTo" id="nyukinYoteibiTo" maxlength="8" />
												</c:otherwise>
											</c:choose>
										</td>
										<td colspan="3"></td>
					...
					<!-- 明細部 -->
					<c:if test="${FormNyu0102d02.showListFlag}">
									...
									<c:otherwise>
										<div id="divBody" class="divBody height475">
											<table id="tblBody" class="tbl_list">
												...
												<%-- START 請求先情報ループ --%>
												<c:forEach items="${FormNyu0102d02.nyuSeikyusakiList}" var="seikyusakiInfo" varStatus="status">
													
													<tr>
														<%-- No --%>
														<td class="contextMenu align_right" id="${status.count}">
														...
														<%-- 請求締日 --%>
														<td>
															<fmt:formatDate value="${ DateUtil.toDate(seikyusakiInfo.seikyuShimebi, 'yyyyMMdd')}" pattern="yyyy/MM/dd"/>
														</td>
														<%-- 入金予定日 --%>
														<td>
															<fmt:formatDate value="${ DateUtil.toDate(seikyusakiInfo.nyukinYoteibi, 'yyyyMMdd')}" pattern="yyyy/MM/dd"/>
														</td>
														<%-- 請求金額 --%>
														...
														<%-- 今回売上額 --%>
														<td class="align_right">
															<fmt:formatNumber value="${seikyusakiInfo.uriageGaku}" pattern="###,###" />
														</td>
														<%-- 未回収残 --%>
														<td class="align_right">
			...
			<form:hidden id="selectIdx"       path="selectIdx" />
			
			<!-- 検索条件 -->
			<form:hidden id="condSelectedJigyoshoCd" path="condSelectedJigyoshoCd" />
			<form:hidden id="condSeikyusakiCdFrom"   path="condSeikyusakiCdFrom" />
			<form:hidden id="condSeikyusakiCdTo"     path="condSeikyusakiCdTo" />
			<form:hidden id="condNyukinYoteibiFrom"  path="condNyukinYoteibiFrom" />
			<form:hidden id="condNyukinYoteibiTo"    path="condNyukinYoteibiTo" />
			...
			<div id="tableMenu" class="sub-menu">
				<a href="#" id="selectSubMenu">
					<img src="${pageContext.request.contextPath}/resources/img/tool_view.png" height="12" />
					選択
				</a>
			</div>
